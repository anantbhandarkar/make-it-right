---
name: mir-mobile
description: "Make It Right (mobile pillar). Constraint-first NATIVE MOBILE planning protocol — AI writes screens that run in the simulator; this makes them RIGHT under process death, permission denial, flaky cellular, OS background limits, and app-store review. Forces explicit constraints before code: what survives an uninstall/reinstall, what happens when the OS kills the process mid-flow, what happens when the user taps Deny (and Deny again), what happens when a POST is retried on a dropped connection. Runs the hard-gated pipeline (Intent → Constraint Interrogation → Assumption Ledger → Invariants & App State Machine → Risk Register → Design Review → Implementation → Production-Readiness + store-submission review). Carries the release-side gates AI ignores entirely: Google Play targetSdk and Play Billing deadlines, restricted-permission declarations, Apple's Xcode/SDK minimum, and PrivacyInfo.xcprivacy required-reason APIs plus third-party SDK signatures. Chains into mir-mobile-android and mir-mobile-ios. TRIGGER for app work that ships to the App Store or Google Play in ANY mobile stack — Swift/SwiftUI, Kotlin/Jetpack Compose, Kotlin Multiplatform + Compose Multiplatform, React Native, Flutter — including background work, offline sync, runtime permissions, keychain/keystore, push, deep links, in-app purchase, and store submission. Also TRIGGER regardless of distribution channel — enterprise/MDM, OEM-preload, internal and sideloaded builds hit the same lifecycle, permission and storage failures. SKIP for mobile web / responsive web / PWA running in a browser (that is mir-frontend) and for the server API the app talks to (mir-backend). Platform library mechanics are DELEGATED, not skipped: mir-mobile-android (Compose, WorkManager, Room, Gradle) and mir-mobile-ios (SwiftUI, SwiftData, BGTaskScheduler, Xcode) load ALONGSIDE this skill, never instead of it — they carry the SDK mechanics, this carries the gates."
trigger: /mir-mobile
