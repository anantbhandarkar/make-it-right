...
description: "Make It Right (vanilla JS / no-framework reactivity tier). Plain-DOM footguns that no reactive library is present to hide. Covers: event listeners never removed (the #1 leak) and AbortController as the removal mechanism, detached DOM nodes retained by a closure or a module-scope map, IntersectionObserver/MutationObserver/ResizeObserver never disconnected, timers that outlive their element, innerHTML as an XSS sink and the current alternatives (textContent, DOM APIs, Element.setHTML + Sanitizer, Trusted Types CSP), layout thrashing from interleaved read/write, DocumentFragment batching, event delegation vs per-node listeners, manual state/DOM divergence and the idempotent render-from-state discipline, custom-element lifecycle and upgrade timing, attribute-vs-property reflection loops, shadow DOM style/focus/ARIA consequences, form-associated custom elements via ElementInternals, stale-response-overwrites-fresh-response fetch races, and manual focus management (focus after route change, native dialog focus trap, aria-live regions). TRIGGER when the UI is built with plain DOM APIs and no reactive library — vanilla JS/TypeScript, jQuery-era code, hand-written Web Components / custom elements, a static site with its own script, a design-system primitive, a browser-extension content script, or a widget embedded in a third-party page. SKIP when a reactive library owns rendering — React (mir-frontend-react), Vue, Angular, Svelte, and Solid each get their own mir-frontend-<lib> tier — and SKIP for the generic UX/state/interrogation gates (mir-frontend) and for build/meta-framework mechanics (Astro islands, Eleventy, Vite config), which belong to a framework module."
