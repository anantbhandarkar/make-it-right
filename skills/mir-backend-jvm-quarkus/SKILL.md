...
description: "Make It Right (Quarkus module). Quarkus 3.x (LTS 3.33) + Hibernate ORM / Panache + Quarkus REST (formerly RESTEasy Reactive) + SmallRye Mutiny reliability footguns specific to this framework stack. Covers: build-time DI / annotation processing (reflection for GraalVM native image must be registered with @RegisterForReflection or fails only at native runtime), the Quarkus REST execution model (the return type picks the thread — Uni/Multi/CompletionStage run on the Vert.x event loop, everything else already runs on a worker thread, and @NonBlocking is the way to break it), Mutiny async composition pitfalls (blocking inside Uni/Multi pipelines), @RunOnVirtualThread, build-time vs runtime config keys and secrets baked into a native binary, Dev Services and Dev UI absent in production, native-image gotchas (dynamic proxies, serialization, static initializers), and Quarkus security mechanics (quarkus.security.jaxrs.deny-unannotated-endpoints defaulting to false, CORS config keyed on quarkus.http.cors.enabled with origins deny-by-default when unset, the quarkus-rest-csrf extension, Panache query parameterization and active-record mass assignment, and the current path-normalization authorization-bypass advisories CVE-2026-39852 and CVE-2026-50559 against quarkus.http.auth.permission policies). Always loads TOGETHER WITH mir-backend (the gates) and mir-backend-jvm (JVM runtime concerns: thread pools, virtual threads and JEP 491, GC, container heap, cold start, JMM visibility, ThreadLocal hygiene, JDK-level security defaults); this module only adds Quarkus library mechanics. TRIGGER only when the JVM backend stack is Quarkus — building, reviewing, or debugging a Quarkus REST resource, CDI bean, Panache entity, Mutiny pipeline, HTTP security policy, or native image build. SKIP for Spring Boot, Micronaut, Vert.x standalone, Helidon, Ktor, or any non-Quarkus JVM framework (those get their own mir-backend-jvm-<framework> module), and for non-JVM runtimes."
