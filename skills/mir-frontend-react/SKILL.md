...
description: "Make It Right (React reactivity tier). React 19 + React Compiler reactivity footguns shared across EVERY React meta-framework (Next.js, React Router 7/Remix, TanStack Start, Vite SPA) — distinct from the generic frontend gates and from any one framework's mechanics. Covers: the Rules of Hooks (including the try/catch/finally ban and the use() exception), effect-dependency discipline (derive in render, effects are for external sync only), useEffectEvent, stale closures, list-key correctness, controlled vs uncontrolled inputs, use() and promise identity, Actions / useActionState / useOptimistic, granular Suspense + Error Boundary placement and the 300 ms reveal throttle, <Activity> hidden-vs-unmounted, useTransition/useDeferredValue for INP, React Compiler 1.0 interop and eslint-plugin-react-hooks v7 (compiler rules are on by default now; the 'use no memo' opt-out), concurrent-rendering/StrictMode double-invoke, the server-state-vs-client-state boundary (TanStack Query, not useState mirrors), and React-layer security (the raw-HTML props, markdown and LLM-output rendering, client-side authorization, secrets in the bundle, the react-server-dom-* advisory floor). TRIGGER when the frontend reactivity library is React — loads TOGETHER WITH mir-frontend (the gates) and, where one exists, the framework module (mir-frontend-react-next). SKIP for Vue/Angular/Svelte, each of which gets its own mir-frontend-<lib> tier. SKIP for meta-framework library mechanics — RSC boundary wiring, Server Actions, file-based routing, framework caching and middleware belong to mir-frontend-react-next, not here. SKIP for non-UI backend code (mir-backend and its runtime tiers own that) — but React Server Components DO trigger this tier, because use(), render purity, promise identity and Suspense placement all apply on the server."
