...
description: "Make It Right (PostgreSQL module). The Postgres engine mechanics the engine-agnostic pillar deliberately omits: which ALTER TABLE subform takes ACCESS EXCLUSIVE and the lock-queue problem where one blocked DDL stalls every read behind it; the lock_timeout/statement_timeout migration discipline; ADD COLUMN with volatile vs non-volatile defaults; ALTER COLUMN TYPE rewrites; adding NOT NULL via a NOT VALID constraint then VALIDATE (including PG 18's NOT VALID NOT NULL); CREATE INDEX CONCURRENTLY and its invalid-index cleanup path; MVCC bloat, autovacuum tuning, XID and multixact wraparound, and the four things that hold back vacuum (long transactions, idle-in-transaction, replication slots, prepared transactions); isolation levels as actually implemented (READ UNCOMMITTED is READ COMMITTED, REPEATABLE READ is snapshot isolation) and 40001/40P01 retry; SELECT FOR UPDATE vs FOR NO KEY UPDATE and the FOR KEY SHARE foreign-key conflict; SKIP LOCKED queue tables; transaction- vs session-scoped advisory locks and deadlock ordering; b-tree/GIN/GiST/BRIN selection, partial and expression indexes, INCLUDE covering indexes and the visibility-map requirement for index-only scans, and why an index is not used; exclusion constraints, generated columns, jsonb-vs-columns, enum alteration cost, numeric for money, timestamptz always; connection cost, PgBouncer pooling modes and exactly what breaks under transaction pooling; declarative partitioning; and RLS multi-tenancy with its FORCE/BYPASSRLS/security_invoker bypass paths and per-row policy cost. TRIGGER when the engine is PostgreSQL or a Postgres-compatible service (RDS/Aurora Postgres, Cloud SQL, Neon, Supabase, Timescale) and the task writes DDL, a migration, an index, a locking or isolation decision, a partitioning or RLS layout, or diagnoses a slow query, an execution plan (EXPLAIN / pg_stat_statements), a missing or unused index, or debugs bloat, vacuum, deadlocks, or a connection pooler. Loads TOGETHER WITH mir-database (the gates). SKIP for MySQL, SQL Server, Oracle, SQLite, MongoDB, DynamoDB and every other engine; for application-side transaction/idempotency/retry code and ORM or migration-tool wiring (Alembic, Prisma, ActiveRecord, GORM, Ecto — those are mir-backend and its framework modules); and for analytics/warehouse dimensional modeling and streaming pipelines."
