---
name: mir-mobile-ios
description: "Make It Right (iOS module). Swift 6 + SwiftUI + Swift Concurrency mechanical footguns on Apple platforms — the ones the platform-agnostic mobile gates deliberately omit. Covers: what the Swift 6 language mode rejects that Swift 5 allowed (non-isolated global mutable state, non-Sendable values crossing isolation boundaries, nonisolated protocol-conformance mismatch), Xcode 26's MainActor-by-default plus the nonisolated(nonsending) / @concurrent change that silently keeps async work ON the main actor, Task lifetime (a Task {} started in a view is NOT cancelled on disappear — only .task is), async let vs TaskGroup, SwiftUI identity and the ForEach(id: \\.self) bug that reuses @State across rows, @State vs @StateObject vs @ObservedObject and the recreated-every-render bug, @Observable's per-property invalidation and reads that escape body tracking, state written during body evaluation, onAppear firing repeatedly, scenePhase and app-switcher snapshot leakage, BGTaskScheduler's non-guarantee and background-task expiration, retain cycles through escaping closures / Combine sinks / delegates / long-lived Tasks, and iOS security specifics (Keychain accessibility classes and backup-restore, UserDefaults, ATS exceptions, pasteboard, PrivacyInfo.xcprivacy required-reason APIs, App Groups, universal links vs URL-scheme hijacking, SPM macro/plugin build-time execution). TRIGGER when the target is an Apple-platform app in Swift/SwiftUI/UIKit — iOS, iPadOS, watchOS, tvOS, visionOS — building or reviewing views, view models, concurrency, background work, Keychain, push, deep links, or an App Store submission. Loads TOGETHER WITH mir-mobile (the gates). In a React Native or Flutter app, SKIP the SwiftUI/UIKit sections while the work is in JS/Dart, but TRIGGER for that app's native iOS layer — Xcode project and build settings, entitlements, Info.plist and background modes, PrivacyInfo.xcprivacy, CocoaPods/SPM dependencies, signing, and App Store submission. SKIP for Android/Kotlin (mir-mobile-android), for mobile web / PWA (mir-frontend), and for the server API the app calls (mir-backend)."
