...
description: "Make It Right (Android module). Kotlin + Jetpack Compose + coroutines mechanics for the mir-mobile pillar — the Android-specific footguns the platform-agnostic skill omits: Activity/Fragment lifecycle and configuration-change recreation, what SavedStateHandle and rememberSaveable actually survive (process death) versus what only survives rotation, viewModelScope vs lifecycleScope, repeatOnLifecycle and collectAsStateWithLifecycle (collecting a flow in onCreate keeps the collector running off-screen and drains battery), cooperative cancellation, Compose recomposition storms from unstable parameters under strong skipping, remember vs rememberSaveable, derivedStateOf, LaunchedEffect/DisposableEffect/SideEffect and the wrong-key bug, state hoisting and per-row state, WorkManager for guaranteed work, the mandatory android:foregroundServiceType declaration and the dataSync 6h/24h cap, Doze and app standby buckets, SCHEDULE_EXACT_ALARM vs USE_EXACT_ALARM, the runtime-permission flow and its permanently-denied branch, photo picker and partial media access, predictive back under targetSdk 36, and Android security mechanics (Keystore keys, why SharedPreferences is not secure storage and security-crypto is deprecated, android:exported, intent redirection, implicit-intent leakage, PendingIntent FLAG_IMMUTABLE, App Links assetlinks.json, WebView addJavascriptInterface and file access, cleartext traffic and network security config, tapjacking, root/tamper limits). TRIGGER when the mobile target is native Android — Kotlin, Jetpack Compose, Android Views/XML, ViewModel, Room, DataStore, WorkManager, Gradle/R8, AndroidManifest, Play Console. Loads TOGETHER WITH mir-mobile (the gates). In a React Native or Flutter app, SKIP the Compose/Kotlin sections while the work is in JS/Dart, but TRIGGER for that app's native Android layer — AndroidManifest, Gradle/AGP and R8, targetSdk migration, foreground-service and permission declarations, signing, and Play Console. SKIP for iOS/Swift/SwiftUI (mir-mobile-ios), for mobile web and PWAs (mir-frontend), and for the server API the app calls (mir-backend)."
