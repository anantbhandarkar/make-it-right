...
3. **Check render-model and tier fitness.** Identify the rendering model (chosen or implied), then consult `references/rendering-model-map.md`. If the workload lands in that model's "Do NOT use when…" column (e.g. CSR chosen for a public marketing page, or RSC chosen for a heavy offline-first app), **say so now** as a conscious, ledgered choice — never a silent default. Then name the tier you will load at Gate 5:
   ...
   |---|---|---|
   | React, any meta-framework | `mir-frontend-react` | `mir-frontend-react-next` (Next.js) · `mir-frontend-react-remix` (React Router 7/8 Framework Mode — the product formerly called Remix). None yet for TanStack Start or a plain Vite React SPA |
...
|---|---|---|
| `mir-frontend-react` | hook rules, derived state, stale closures, Suspense + Error Boundary placement, Actions, Compiler interop | `mir-frontend-react-next` — RSC boundary, Server Actions, `use cache`, revalidation · `mir-frontend-react-remix` — React Router loaders/actions, middleware, single fetch, revalidation |
...
**Still uncovered on React:** TanStack Start and a plain Vite React SPA have no framework module. If the stack is one of those, run this pillar plus `mir-frontend-react`, and record in the design that module-level rules were unavailable. **Remix 3 is not one of these stacks** — it drops React for a forked Preact, so neither `mir-frontend-react` nor `mir-frontend-react-remix` (which covers React Router / Remix v2) applies; run this pillar alone and say so.
...
> **"Does it only apply to one meta-framework?"** → **framework module** (edit `mir-frontend-react-next`, `mir-frontend-react-remix`, or `mir-frontend-vue-nuxt`). Angular is the exception — it has no module, so its framework mechanics (DI, Router, Forms, `@angular/ssr`, the CLI) live in `mir-frontend-angular` and its `references/`.
...
| **Framework module** | one meta-framework's mechanics | `skills/mir-frontend-react-next/` · `skills/mir-frontend-react-remix/` · `skills/mir-frontend-vue-nuxt/` (SKILL.md + `references/`) | the rule is a **mechanical footgun of one framework** — RSC boundary, Server Action authorization, `use cache`, React Router loaders/actions and middleware, `useAsyncData` keys, file-routing waterfall |
...
Still **not written**, so nothing can load them: a TanStack Start module and a plain-Vite-React-SPA module. Those stacks get this pillar plus `mir-frontend-react`. Writing one is a new directory, not an edit here.
...
**Currency baseline — checked against the npm registry on 13 Aug 2026:** react/react-dom 19.2.8 · next 16.3.0 · react-router 8.3.0 (re-checked 25 Aug 2026; `mir-frontend-react-remix` owns the React Router patch floors and the v6/Remix-v2 EOL facts) · @tanstack/react-start 1.168.44 · @tanstack/react-query 5.101.4 · @tanstack/react-virtual 3.14.9 · vue 3.5.41 · nuxt 4.5.2 · @angular/core 22.1.3 and @angular/cli 22.1.5 (both re-checked 25 Aug 2026; `mir-frontend-angular` owns the Angular floor and its LTS dates) · svelte 5.56.9 · vite 8.2.2 (re-checked 25 Aug 2026) · astro 7.2.1 · tailwindcss 4.3.3 · dompurify 3.4.13 · axe-core 4.13.0 · web-vitals 6.1.0 · eslint-plugin-react-hooks 7.1.1 · babel-plugin-react-compiler 1.0.0. Standards: WCAG 2.2 is a W3C Recommendation (updated 12 Dec 2024); Core Web Vitals are LCP ≤ 2.5 s, INP ≤ 200 ms, CLS ≤ 0.1, with FID retired in favour of INP. **Anything not on that list is unverified here — check it before you quote it.** Per-package security floors live in the tier and module skills, which own them.
