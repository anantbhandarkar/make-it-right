...
description: "Make It Right (Next.js module). Next.js 16 App Router mechanics — the footguns that exist only in this meta-framework, not in React generally. Carries: the Server/Client Component boundary and how one 'use client' pulls its entire import graph into the browser bundle; Server Actions as public POST endpoints that must re-authenticate and re-authorize on every call (hiding the button is not access control) plus argument validation and return-value filtering; proxy.ts (formerly middleware.ts) as an optimistic redirect and never the sole auth gate — this framework has a repeating middleware-bypass advisory class (CVE-2025-29927, CVE-2026-45109, CVE-2026-64642); the caching layers after Next 16 made caching opt-in ('use cache', cacheComponents, cacheLife, cacheTag) and the fact that cache keys and tags are stored in plain text; revalidateTag vs updateTag vs revalidatePath vs refresh; request waterfalls from sequential awaits in nested layouts; Suspense placement and what it does to the prerendered shell; what silently opts a route into dynamic rendering; NEXT_PUBLIC_ variables inlined into the client bundle at build time; Route Handlers vs Server Actions; and next/image + next/font correctness (CLS, remotePatterns, sizes, module-scope loaders). TRIGGER only when the React meta-framework is Next.js — work in app/, page.tsx, layout.tsx, route.ts, proxy.ts or middleware.ts, any 'use server' file, next.config.ts, or any Next.js caching, rendering, revalidation or deployment question. Always loads TOGETHER WITH mir-frontend (the gates) and mir-frontend-react (hooks, effects, Compiler interop); this module adds only Next.js library mechanics. SKIP for React Router 7 / Remix, TanStack Start, Astro, plain Vite SPAs, Nuxt and SvelteKit, and for React-general reactivity rules (Rules of Hooks, stale closures, derived state, useMemo/Compiler interop — those live in the React tier, not here)."
...
4. A *different* React meta-framework (React Router 7, TanStack Start)? → its own `mir-frontend-react-<framework>` module. Never widen this one.
