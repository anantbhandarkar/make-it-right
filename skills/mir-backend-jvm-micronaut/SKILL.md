...
description: "Make It Right (Micronaut module). Micronaut 5.x (Java 25 baseline) / 4.x + Micronaut Data + Micronaut Security + Netty HTTP server reliability footguns specific to this framework stack. Covers: compile-time DI and AOT (bean definitions are generated at build, but candidate resolution is still runtime — NoSuchBeanException/NonUniqueBeanException can surface after startup because singletons are lazy), bean scope pitfalls (@Singleton default, don't hold request state), blocking the Netty event loop (@ExecuteOn(TaskExecutors.BLOCKING), which is virtual-thread backed when the JVM supports it, or reactive types), Micronaut Data repository transaction scoping and self-invocation, compile-time AOP interceptor limits (final/private/new-ed instances), native-image / GraalVM considerations at the Micronaut layer, and Micronaut security mechanics (CORS wide-open when enabled with no configurations plus allowCredentials defaulting to true on 4.x and false on 5.x, @Secured object-level authorization, Micronaut Data query binding, and the July 2026 HTTP-client advisories GHSA-q6gh-6v2r-hjv3 cross-origin credential leakage and GHSA-387m-935m-c4vw unbounded-redirect DoS). Always loads TOGETHER WITH mir-backend (the gates) and mir-backend-jvm (JVM runtime concerns: thread pools, virtual threads and JEP 491, GC, container heap, cold start, JMM visibility, ThreadLocal hygiene, JDK-level security defaults); this module only adds Micronaut library mechanics. TRIGGER only when the JVM backend stack is Micronaut — building, reviewing, or debugging a Micronaut controller, service, repository, filter, HTTP client, or security rule. SKIP for Spring Boot, Quarkus, Vert.x standalone, Helidon, Ktor, or any non-Micronaut JVM framework (those get their own mir-backend-jvm-<framework> module), and for non-JVM runtimes."
