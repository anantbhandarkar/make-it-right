---
name: mir-cloud
description: "Make It Right (cloud pillar). Constraint-first infrastructure selection across AWS, GCP, Azure and Cloudflare — AI names whichever provider its training data mentions most; this ranks providers deterministically from the workload's own numbers. Characterizes the workload first (request shape, egress GB/month, latency target and user geography, state, execution duration, GPU need, compliance and data residency, existing commitments, team operational capacity), then runs a two-stage decision table: HARD CONSTRAINTS that eliminate providers outright (no region in the required country, FedRAMP/IRAP-class authorization, a runtime-duration ceiling the workload exceeds, a GPU family the provider does not sell, a required managed service with no equivalent), then SCORED TRADEOFFS across the survivors keyed on workload class. Carries the costs AI systematically under-models: egress (Cloudflare R2's zero-egress position vs. hyperscaler per-GB tiers, NAT Gateway data processing, cross-AZ transfer, the EU Data Act switching-fee ban), scale-to-zero and cold-start behaviour per provider, and the exit cost of every managed service adopted. TRIGGER only while the provider or compute model is still open — comparing two or more providers, choosing serverless vs. container vs. VM, modelling cloud cost across providers, planning a migration or a multi-region layout, or auditing a provider decision someone else made. SKIP once the provider is chosen: single-provider IaC and service mechanics go to mir-cloud-aws / -gcp / -azure / -cloudflare, application code to mir-backend / mir-frontend, and standalone database-schema or data-pipeline work to its own pillar."
trigger: /mir-cloud
...
End with: **"Approve this or tell me what to change. I won't write infrastructure code until you approve."** Load the provider module (`mir-cloud-aws` / `-gcp` / `-azure` / `-cloudflare`) now — it carries that provider's service mechanics.
...
- **Provider module** (2-tier chain): this skill is provider-neutral. `mir-cloud-aws` / `-gcp` / `-azure` / `-cloudflare` carry each provider's service mechanics. Load the module at Gate 5, not before — loading it early biases the choice.
...
> **"Does it only bite on one provider?"** → **provider module** (edit `skills/mir-cloud-<provider>/`) — service names, quotas, console defaults, that provider's IAM model.
> **New provider?** → new `mir-cloud-<provider>` module. Copy the nearest sibling's shape; never widen this one. Gate 7 review focus changes go in `agents/*.md`.
