...
description: "Make It Right (Echo module). Echo web framework reliability augmentation for Go backends, covering both Echo v5 (current) and v4 (maintained). Use alongside mir-backend and mir-backend-go when the target stack is Echo — adds the mechanical footguns the runtime-agnostic tiers deliberately omit: echo.Context comes from a sync.Pool and must never be retained past the handler or handed to a goroutine; the v4→v5 rewrite (Context became a *echo.Context struct, Logger became *slog.Logger, HTTPErrorHandler's arguments swapped, e.Shutdown and e.Close were removed in favour of StartConfig) that AI mixes up; Bind never validates, so a missing c.Validate call ships unchecked input; Bind also merges path and query values into the same struct, which is a mass-assignment path; middleware ordering (Recover before your middleware, e.Pre before routing vs e.Use after); HTTPErrorHandler shape and DefaultHTTPErrorHandler(true) leaking internal errors; graceful shutdown; and Echo's insecure defaults — c.RealIP() trusts X-Forwarded-For unless IPExtractor is set, and middleware.Secure sends no HSTS or CSP. TRIGGER only when the Go backend uses the Echo framework — building, reviewing, or debugging an Echo handler, middleware, or router. Always loads TOGETHER WITH mir-backend (generic gates) and mir-backend-go (Go runtime concerns: goroutine leaks, context propagation, data races, channel rules, typed-nil, defer-in-loop, slice aliasing, error discipline, WaitGroup, os.Root, module checksums); this module only adds Echo library mechanics. SKIP for Gin (mir-backend-go-gin), Fiber (mir-backend-go-fiber), chi, stdlib net/http, or any non-Echo stack, and for non-Go runtimes."
